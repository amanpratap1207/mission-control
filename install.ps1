...
<#
.SYNOPSIS
    Ramtri Solutions — Windows Installer
    The mothership for your OpenClaw fleet.

.DESCRIPTION
    Installs Ramtri Solutions on Windows via local Node.js deployment.
...
.PARAMETER InstallDir
    Target directory when cloning from GitHub (default: .\ramtri-solutions).
...
# ── Defaults ──────────────────────────────────────────────────────────────────
if (-not $InstallDir) {
    $InstallDir = if ($env:MC_INSTALL_DIR) { $env:MC_INSTALL_DIR } else { Join-Path (Get-Location) "ramtri-solutions" }
}
$RepoUrl = "https://github.com/builderz-labs/ramtri-solutions.git"
...
function Get-Source {
    if (Test-Path (Join-Path $script:InstallDir ".git")) {
        ...
        Write-MC "Cloning Ramtri Solutions..."
...
function Deploy-Docker {
    ...
    try {
        ...
        Write-MC "Waiting for Ramtri Solutions to become healthy..."
        $retries = 30
        while ($retries -gt 0) {
            try {
        ...
        if ($retries -eq 0) {
            ...
            Write-Ok "Ramtri Solutions is running in Docker"
        }
    } finally {
        Pop-Location
    }
...
function Deploy-Local {
    ...
    try {
        ...
        Write-MC "Building Ramtri Solutions..."
        pnpm build
        ...
        Write-MC "Starting Ramtri Solutions..."
        ...
        if (-not $process.HasExited) {
            Write-Ok "Ramtri Solutions running (PID $($process.Id))"
        } else {
...
function Test-OpenClaw {
    ...
    if (Test-Path $ocHome) {
        ...
        if (Test-Path $ocConfig) {
            ...
            Write-MC "Ramtri Solutions will create a default config on first gateway connection"
        }
    } else {
...
function Main {
    ...
    Write-Host "  |   Ramtri Solutions Installer          |" -ForegroundColor Magenta
    ...
    if ((Test-Path $packageJson) -and (Select-String -Path $packageJson -Pattern '"ramtri-solutions"' -Quiet)) {
