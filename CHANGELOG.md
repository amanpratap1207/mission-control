...
All notable changes to Ramtri Solutions are documented in this file.
...
Ramtri Solutions 2.0.1 is the first patch release after the v2 launch. It rolls up the full set of fixes and follow-on features that landed after `v2.0.0`, including HTTP/Tailscale login hardening, zero-config onboarding, internationalization, gateway/runtime stability fixes, deeper task-routing automation, and the latest OpenClaw compatibility updates.
...
- Stopped forcing `dangerouslyDisableDeviceAuth` during Ramtri Solutions gateway registration
...
- Ramtri Solutions builds now isolate build-time SQLite state from runtime SQLite state, eliminating `SQLITE_BUSY` build contention
...
- README, landing-page handoff, and release documentation refreshed to match the current Ramtri Solutions interface and feature set
- This release captures 189 commits on top of `main` and marks the major refactor branch as the new baseline for Ramtri Solutions
