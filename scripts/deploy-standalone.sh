...
migrate_runtime_data_dir() {
  local target_data_dir="${RAMTRI_SOLUTIONS_DATA_DIR:-$SOURCE_DATA_DIR}"
  ...
  local source_db="$SOURCE_DATA_DIR/ramtri-solutions.db"
  local target_db="$target_data_dir/ramtri-solutions.db"
  ...
  if command -v sqlite3 >/dev/null 2>&1; then
    ...
    if [[ -f "$SOURCE_DATA_DIR/ramtri-solutions-tokens.json" ]]; then
      cp "$SOURCE_DATA_DIR/ramtri-solutions-tokens.json" "$target_data_dir/ramtri-solutions-tokens.json"
    fi
    ...
    rsync -a \
      --exclude 'ramtri-solutions.db-shm' \
      --exclude 'ramtri-solutions.db-wal' \
      --exclude '*.db-shm' \
      --exclude '*.db-wal' \
...
RAMTRI_SOLUTIONS_DATA_DIR="$BUILD_DATA_DIR" \
RAMTRI_SOLUTIONS_DB_PATH="$BUILD_DATA_DIR/ramtri-solutions.db" \
RAMTRI_SOLUTIONS_TOKENS_PATH="$BUILD_DATA_DIR/ramtri-solutions-tokens.json" \
pnpm build
