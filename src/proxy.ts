...
import { buildRamtriSolutionsCsp, buildNonceRequestHeaders } from '@/lib/csp'
...
function addSecurityHeaders(response: NextResponse, _request: NextRequest, nonce?: string): NextResponse {
  ...
  response.headers.set('Content-Security-Policy', buildRamtriSolutionsCsp({ nonce: effectiveNonce, googleEnabled }))
