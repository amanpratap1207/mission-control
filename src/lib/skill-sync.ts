...
function getSkillRoots(): Array<{ source: string; path: string }> {
  ...
  const roots: Array<{ source: string; path: string }> = [
    ...
    { source: 'workspace', path: process.env.MC_SKILLS_WORKSPACE_DIR || join(process.env.OPENCLAW_WORKSPACE_DIR || process.env.RAMTRI_SOLUTIONS_WORKSPACE_DIR || join(openclawState, 'workspace'), 'skills') },
  ]
