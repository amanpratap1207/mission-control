# Ramtri Solutions Platform Hardening + Full Agent CLI/TUI PRD
...
Goal
Build a production-grade Ramtri Solutions operator surface for autonomous agents via a first-party CLI (and optional lightweight TUI), while fixing platform inconsistencies discovered in audit: API contract drift, uneven reliability controls, and incomplete automation ergonomics.

Architecture
Ramtri Solutions remains the source of truth with REST + SSE endpoints. A first-party CLI consumes those APIs with profile-based auth and machine-friendly output. TUI is layered on top of CLI API client primitives for shared behavior. API contract reliability is enforced through route-to-spec parity checks in CI.
...
Current Ramtri Solutions backend has strong capabilities for agent orchestration, but external automation quality is constrained by:
...
Result: agents can use Ramtri Solutions partially, but not yet with high confidence as a full control plane.
...
Integration
- CLI against local Ramtri Solutions test server.
