...
Comprehensive security hardening guide for Ramtri Solutions and OpenClaw Gateway deployments.
...
Ramtri Solutions uses a host allowlist in production:
...
Ramtri Solutions sets these headers automatically:
...
Ramtri Solutions acts as the mothership for your OpenClaw fleet. The installer automatically checks and repairs common OpenClaw configuration issues.
...
- Audit third-party skills before installing (Ramtri Solutions's Skills Hub runs automatic security scans).
...
- Use Ramtri Solutions's diagnostics API to monitor gateway health.
...
[Reverse Proxy (Caddy/nginx) + TLS]
  |
[Ramtri Solutions :3000] ---- [SQLite .data/]
  |
...
- Ramtri Solutions listens on localhost or a private network
...
Ramtri Solutions logs a security event (`global_api_key_used`) every time the global API key is used. Monitor these in the audit log:
