...
Get from zero to a working agent loop with nothing but Ramtri Solutions and `curl`. No gateway, no OpenClaw, no extra dependencies.

## Prerequisites

- Ramtri Solutions running (`pnpm dev` or Docker)
- An admin account (visit `/setup` on first run)
- Your API key (auto-generated on first run, shown in Settings)

## Step 1: Start Ramtri Solutions
...
Agents can self-register via the API. This is how autonomous agents announce themselves to Ramtri Solutions:
...
Heartbeats tell Ramtri Solutions your agent is alive. Without them, agents are marked offline after 10 minutes:
